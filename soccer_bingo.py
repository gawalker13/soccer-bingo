...
# Dropdown of all timezones
user_tz_input = st.selectbox(
    "Select your timezone:",
    pytz.all_timezones,
    index=pytz.all_timezones.index("America/New_York")  # default selection
)

def to_local(utc_str: str, tz: str):
    """Convert FotMob UTC string → user local timezone datetime."""
    dt = datetime.fromisoformat(utc_str.replace("Z", "+00:00"))
    return dt.astimezone(ZoneInfo(tz))

def get_local_today_key(user_tz: str):
    """
    FotMob keys are based on date, but match filtering must be done
    using the *local* day of each converted match.
    So we fetch both today and tomorrow in UTC to be safe.
    """
...
async def get_todays_games():
    """
    Fetch matches for the user's local date.
    Converts each match's UTC timestamp to user timezone and
    keeps only the matches that occur on the user's local calendar day.
    """
    user_tz = user_tz_input

    # We fetch two days in UTC because late-night EST games show up as next day in UTC
...
with st.expander("Click Here"):
  st.markdown(
      """
      **How to play**
      1. Select your timezone to get today's games.
      2. Pick a game from today's fixtures.
      3. Create 24 custom bingo squares using players, teams, or game events.
      4. If a player is substituted before halftime, the substitute counts.
      5. Click the ✅ below squares as events happen.
      6. For confirmation on the stats, please go to fotmob.com and look for the game(s) you picked. Then look for your player(s) and look for the stat(s)
